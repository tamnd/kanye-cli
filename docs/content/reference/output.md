...
describes what users get. Pick a format with `-o`, or let kanye choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kanye <command> -o table   # aligned columns for reading
kanye <command> -o jsonl   # one JSON object per line, for piping
kanye <command> -o json    # a single JSON array
kanye <command> -o csv     # spreadsheet friendly
kanye <command> -o tsv     # tab-separated
kanye <command> -o url     # just the URL column
kanye <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kanye <command> --fields id,title,url
```
...
```bash
kanye <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kanye <command>            # a table, because this is a terminal
kanye <command> | wc -l    # JSONL, because this is a pipe
```
